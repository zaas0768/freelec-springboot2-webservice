...
// 1. registrationId -> 현재 로그인 진행 중인 서비스를 구분하는 코드이다,
// 지금은 구글만 사용하는 불필요한 값이지만, 이후 네이버 로그인 연동 시에 네이버 로그인인지, 구글 로그인인지 구분하기 위해서 사용한다.
// 2. userNameAttributeName -> OAuth2 로그인 진행 시 키가 되는 필드값을 이야기한다. Primary Key와 같은 의미이다.
// 구글의 경우 기본적으로 코드를 지원하지만, 네이버 카카오 등은 기본 지원하지 않는다. 구글의 기본 코드는 "sub"이다.
// 3. OAuthAttributes -> OAuth2UserService를 통해 가져온 OAuth2User의 attribute를 담을 클래스이다. 이후 네이버 등 다른 소셜 로그인에도 사용한다.
// 4. SessionUser -> 세션에 사용자 정보를 저장하기 위한 Dto 클래스이다. 새로 만들어서 사용